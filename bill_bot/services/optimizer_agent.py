...
class TradeOptimizerAgent:
    ...
    async def analyze_history(self, user_id: int) -> dict:
        ...
        for t in trades:
            ...
            by_pair[pair]["total_pnl"] += pnl

            # By Setup (Pair + TF + Side)
            setup_key = f"{pair}:{tf}:{side}"
            if setup_key not in by_setup:
                by_setup[setup_key] = {"wins": 0, "losses": 0, "total_pnl": 0.0}
            if is_win:
                by_setup[setup_key]["wins"] += 1
            else:
                by_setup[setup_key]["losses"] += 1
            by_setup[setup_key]["total_pnl"] += pnl
